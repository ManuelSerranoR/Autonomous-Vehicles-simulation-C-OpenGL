...
Car::Car(vector<double> vectorForStraightPart, vector<double> vectorForCurvedPart, int color_R, int color_G, int color_B, float initial_positionX, float initial_positionY)
{
	...
	deviationVectorCounterStraight = 0;
	deviationVectorCounterCurved = 0;

	deviationVectorStraight = vectorForStraightPart;
	deviationVectorCurved = vectorForCurvedPart;
...
void Car::Movement(float angle_sample1, float angle_sample2, float angle_sample3, float angle_sample4)
{
	if ((deviationVectorCounterStraight < (deviationVectorStraight.size() - 1)) && (deviationVectorCounterCurved < (deviationVectorCurved.size() - 1)))
	{	
		//STRAIGHT PART
		if (futurePosition.y < 78.9)
		{
			realPosition.x = futurePosition.x;
			realPosition.y = futurePosition.y;

			deviationVectorCounterStraight++;

			futurePosition.x = position.x;
			futurePosition.y = position.y + (0.3*deviationVectorStraight[deviationVectorCounterStraight]);
		...
		//FIRST PART OF DISCRETIZATION OF CLOTHOID
		else if (futurePosition.y < 93.906)
		{
			if (aux==true)
			{
				position.x = 400 + 400 * cos(currentAngle); //The first "0" is because the road is centered in (0,0)
				position.y = 78.9 + 400 * sin(currentAngle); //The first "0" is because the road is centered in (0,0)

				realPosition.x = position.x;
				realPosition.y = position.y;

				currentAngle = currentAngle - (angle_sample1);

				futurePosition.x = position.x + (cos(currentAngle) * 3.5 * deviationVectorCurved[deviationVectorCounterCurved]);
				futurePosition.y = position.y + (sin(currentAngle) * 3.5 * deviationVectorCurved[deviationVectorCounterCurved]);
				
				//The rotation angle depends on the current and future location in order to get to know the current angle the car is heading to
				rotationAngle = -90 + (180 / PI)*atan((futurePosition.y - realPosition.y) / (futurePosition.x - realPosition.x)); // degrees (not radians) being Y axis our 0 degrees
				aux = false;
			}
		...
		//SECOND PART OF DISCRETIZATION OF CLOTHOID
		else if (futurePosition.y < 108.87)
		{
			if (aux1 == true)
			{
				position.x = 200.141 + 200 * cos(currentAngle); //The first "0" is because the road is centered in (0,0)
				position.y = 86.403 + 200 * sin(currentAngle); //The first "0" is because the road is centered in (0,0)

				realPosition.x = position.x;
				realPosition.y = position.y;

				currentAngle = currentAngle - (angle_sample2);
		...
		//THIRD PART OF DISCRETIZATION OF CLOTHOID
		else if (futurePosition.y < 123.65)
		{
			if (aux2 == true)
			{
				position.x = 133.896 + 133.3333 * cos(currentAngle); //The first "0" is because the road is centered in (0,0)
				position.y = 93.892 + 133.3333 * sin(currentAngle); //The first "0" is because the road is centered in (0,0)

				realPosition.x = position.x;
				realPosition.y = position.y;

				currentAngle = currentAngle - (angle_sample3);
		...
		//FOURTH PART OF DISCRETIZATION OF CLOTHOID
		else if (futurePosition.y < 137.984)
		{
			if (aux3 == true)
			{
				position.x = 101.404 + 100 * cos(currentAngle); //The first "0" is because the road is centered in (0,0)
				position.y = 101.334 + 100 * sin(currentAngle); //The first "0" is because the road is centered in (0,0)

				realPosition.x = position.x;
				realPosition.y = position.y;

				currentAngle = currentAngle - (angle_sample4);
